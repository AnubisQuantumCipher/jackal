# JACKAL Hermes plugin — proof-carrying range bounds

A load-bearing Hermes / MCP-style plugin exposing exactly two tools:

| Tool | Effect |
|---|---|
| `jackal_range_bound` | Emits a `jackal-formal-receipt-v1` JSON receipt with the certificate embedded, or a stable refusal class. |
| `jackal_verify_receipt` | Re-runs the pinned Lean-proved checker over an embedded certificate; returns `verified` or a stable refusal class. |

Both tools thread requests through **the same** pinned executables the
`jackal-cert-release` gate does (`jackal-native` + `jackal_cert_check`), the
same shared validator (`tests/release_validate.py`), the same formal-status
gate (`tools/formal_status_gate.py`) and the same coverage inventory
(`release/coverage/formal_coverage_inventory.json`).  The plugin adds no
...
* `plugin/hermes/jackal_hermes http --port 8181` — tiny HTTP server
  wrapping the same two tools (POST `/tools/<name>` with a JSON body).
