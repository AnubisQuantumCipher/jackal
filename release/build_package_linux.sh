#!/bin/sh
# Build the additive JACKAL v1.7.3 Linux aarch64 package (adapted copy).
#
...
if [ "$SYSTEM" != "Linux" ] || [ "$MACHINE" != "aarch64" ]; then
  echo "PACKAGE_V173_REFUSED reason=unsupported-host system=$SYSTEM machine=$MACHINE expected=Linux/aarch64" >&2
  exit 3
fi
...
PKG_NAME="jackal-v1.7.3-linux-aarch64"
...
[ -n "$COMPILER" ] || {
  echo "PACKAGE_V173_REFUSED reason=compiler-path-unset" >&2
  exit 4
}
...
# Atomic no-clobber publish: macOS renamex_np(RENAME_EXCL) / Linux
# renameat2(AT_FDCWD, ..., RENAME_NOREPLACE) — same fail-on-EEXIST guarantee.
if platform.system() == "Darwin":
    fn = libc.renamex_np
    fn.argtypes = [ctypes.c_char_p, ctypes.c_char_p, ctypes.c_uint]
    fn.restype = ctypes.c_int
    ctypes.set_errno(0)
    rc = fn(source, destination, 0x00000004)  # RENAME_EXCL
else:
    AT_FDCWD = -100
    ...
    fn.restype = ctypes.c_int
    ctypes.set_errno(0)
    rc = fn(AT_FDCWD, source, AT_FDCWD, destination, RENAME_NOREPLACE)
if rc != 0:
...
release/evidence/int_cert_proof_identity_v172.linux-aarch64.json
release/evidence/gaussian_proof_identity.linux-aarch64.json
release/evidence/lean_admission_audit_v173.json
release/evidence/lean_admission_audit_v173.linux-aarch64.json
release/capability_inventory_v1.json
release/compat/v172_floor.json
release/compat/v172_floor.linux-aarch64.json
...
JACKAL_ANUBIS_COMPILER_PATH="$COMPILER" \
  python3 -I -S -B "$ROOT/release/tools/repin_linux.py" --check >/dev/null
...
/bin/mkdir -p "$DIST"
RELEASE_DEVICE=$(/usr/bin/stat -c '%d' "$ROOT/release")
DIST_DEVICE=$(/usr/bin/stat -c '%d' "$DIST")
...
copy_file "$ROOT/release/evidence/range_proof_identity_v172.linux-aarch64.json" "$PKG/range_proof_identity.json"
copy_file "$ROOT/release/evidence/int_cert_proof_identity_v172.linux-aarch64.json" "$PKG/int_cert_proof_identity.json"
copy_file "$ROOT/release/evidence/gaussian_proof_identity.linux-aarch64.json" "$PKG/gaussian_proof_identity.json"
copy_file "$ROOT/release/evidence/lean_admission_audit_v173.linux-aarch64.json" "$PKG/evidence/lean_admission_audit_v173.json"
...
copy_file "$ROOT/release/compat/v172_floor.linux-aarch64.json" "$PKG/evidence/compat_v172_floor.json"
...
# formal_receipt.py is part of the hermes runtime bundle; bake its Linux compat
# pins BEFORE the bundle digest is computed so MANIFEST and selftest agree.
python3 -I -S -B "$ROOT/release/tools/bake_linux_compat_pins.py" \
  "$PKG/formal_receipt.py" "$RANGE_IDENTITY_FILE_ID" "$INT_IDENTITY_FILE_ID" >/dev/null
PLUGIN_ID=$(python3 -I -S -B "$PKG/plugin/hermes/bundle_hash.py" print)

cat > "$PKG/MANIFEST.sha256" <<EOF
# JACKAL $VER package manifest — Linux aarch64 (locally built); current proof identities schema v2
version $VER
platform $PLATFORM
...
compiler_pin anubis-linux $COMPILER_SHA256
