#!/bin/sh
# Build the deterministic JACKAL v1.7.0 macOS arm64 release package
# (v1.5.0 computation floor + v1.6.0 claim-bundle evidence kernel +
# additive certified composed-integral lane).
# Generated from build_package_v160.sh for the v1.7.0 epoch; the v1.6.0
...
cp "$INT_CERT_CHECKER" "$PKG/jackal_int_cert_check"
...
cp "$ROOT/tools/int_cert_producer.py" "$PKG/int_cert_producer.py"
cp "$ROOT/tools/int_cert_release.py" "$PKG/int_cert_release.py"
...
cp "$ROOT/release/evidence/int_cert_proof_identity.json" "$PKG/int_cert_proof_identity.json"
...
chmod +x "$PKG/int_cert_producer.py" "$PKG/int_cert_release.py"
...
#!/bin/sh
# JACKAL v1.7.0 certified composed-integral gate (self-contained).
set -eu
ROOT=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
[ "$#" -eq 5 ] || { echo "usage: jackal-int-cert-release \"<expr in x>\" <lo> <hi> <tol> <receipt.json>" >&2; exit 2; }
EP=$(awk '/^int_cert_producer /{print $3}' "$ROOT/MANIFEST.sha256")
EC=$(awk '/^int_cert_checker /{print $3}' "$ROOT/MANIFEST.sha256")
EI=$(awk '/^coverage_inventory /{print $3}' "$ROOT/MANIFEST.sha256")
EPF=$(awk '/^int_cert_proof_identity /{print $3}' "$ROOT/MANIFEST.sha256")
EPD=$(awk '/^int_cert_proof_digest /{print $2}' "$ROOT/MANIFEST.sha256")
[ -n "$EP" ] && [ -n "$EC" ] && [ -n "$EI" ] && [ -n "$EPF" ] && \
  [ -n "$EPD" ] || { echo "status=refused reason=manifest-incomplete" >&2; exit 3; }
exec python3 -I -S -B "$ROOT/isolated_entry.py" int-cert \
  --expression "$1" --lower "$2" --upper "$3" --tolerance "$4" \
  --producer "$ROOT/int_cert_producer.py" --checker "$ROOT/jackal_int_cert_check" \
  --expected-producer "$EP" --expected-checker "$EC" \
  --receipt "$5" \
  --inventory "$ROOT/formal_coverage_inventory.json" --expected-inventory "$EI" \
  --proof-identity "$ROOT/int_cert_proof_identity.json" \
  --expected-proof-identity-file "$EPF" --expected-proof-identity-digest "$EPD" \
  --release-epoch v1.7.0
WRAP
chmod +x "$PKG/jackal-int-cert-release"
...
cp "$ROOT/release/evidence/int_cert_proof_identity.json" "$PKG/evidence/"
...
INT_CERT_PRODUCER_ID=$(shasum -a 256 "$PKG/int_cert_producer.py" | awk '{print $1}')
INT_CERT_CHECKER_ID=$(shasum -a 256 "$PKG/jackal_int_cert_check" | awk '{print $1}')
INT_CERT_PROOF_FILE_ID=$(shasum -a 256 "$PKG/int_cert_proof_identity.json" | awk '{print $1}')
INT_CERT_PROOF_ID=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["identity_digest_sha256"])' "$PKG/int_cert_proof_identity.json")
...
int_cert_producer int_cert_producer.py $INT_CERT_PRODUCER_ID
int_cert_checker jackal_int_cert_check $INT_CERT_CHECKER_ID
int_cert_proof_identity int_cert_proof_identity.json $INT_CERT_PROOF_FILE_ID
int_cert_proof_digest $INT_CERT_PROOF_ID
EOF
cat > "$PKG/NON-CLAIMS.txt" <<'EOF'
JACKAL v1.7.0 — explicit non-claims
...
- bound_step composition is mechanized ONLY via the integrate-bound-cert
  certificate lane (int_cert_sound); the engine float lane stays
  implementation-tested-not-mechanized.  Anubis source->native refinement
  remains OPEN.
...
JACKAL v1.7.0 — certified composed integral (int_cert_sound) + claim-bundle evidence kernel + proof-carrying formal receipts + pure-ℚ sqrt/exp/ln/sin/cos/atan/tanh + certified exact CAS (macOS arm64, public, unsigned)
...
Verify, then release a certified enclosure:
  ...
  ./jackal-int-cert-release "x^2" 0 1 1/1000 int-cert-receipt.json  # certified composed integral (v1.7.0, int_cert_sound)
...
The bundled plugin/hermes adapter (34 tools) exposes the same release and
verification path.  See NON-CLAIMS.txt for the exact scope.
EOF
...
int-cert-producer int_cert_producer.py $INT_CERT_PRODUCER_ID
int-cert-checker jackal_int_cert_check $INT_CERT_CHECKER_ID
lean-theorems cert_check_sound cert_encloses certified_release gaussian_integral_check_sound int_cert_sound
