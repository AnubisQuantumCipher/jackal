#!/bin/sh
# Build the additive JACKAL v1.7.2 Apple Silicon macOS package.
#
...
if [ "$SYSTEM" != "Darwin" ] || [ "$MACHINE" != "arm64" ]; then
  echo "PACKAGE_V172_REFUSED reason=unsupported-host system=$SYSTEM machine=$MACHINE expected=Darwin/arm64" >&2
  exit 3
fi
...
PKG_NAME="jackal-v1.7.2-macos-arm64"
...
require_regular() {
  [ -f "$1" ] && [ ! -L "$1" ] || {
    echo "PACKAGE_V172_REFUSED reason=required-regular-file path=$1" >&2
    exit 4
  }
}
...
[ "$OBSERVED_COMPILER_SHA256" = "$COMPILER_SHA256" ] || {
  echo "PACKAGE_V172_REFUSED reason=compiler-authority-drift observed=$OBSERVED_COMPILER_SHA256 expected=$COMPILER_SHA256" >&2
  exit 4
}
...
python3 "$ROOT/release/tools/repin_v172.py" --check >/dev/null

MODE=${1:-}
if [ "$MODE" = "--dry-run" ]; then
  echo "PACKAGE_V172_DRY_RUN_PASS version=$VER platform=$PLATFORM"
...
if [ "$MODE" != "--build" ] || [ "$#" -ne 1 ]; then
  echo "usage: release/build_package_v172.sh --dry-run|--build" >&2
  exit 2
fi

[ ! -e "$FINAL_PKG" ] && [ ! -L "$FINAL_PKG" ] || {
  echo "PACKAGE_V172_REFUSED reason=output-exists path=$FINAL_PKG" >&2
  exit 5
}
[ ! -e "$FINAL_TARBALL" ] && [ ! -L "$FINAL_TARBALL" ] || {
  echo "PACKAGE_V172_REFUSED reason=output-exists path=$FINAL_TARBALL" >&2
  exit 5
}

STAGE=$(mktemp -d "$ROOT/release/.v172-stage.XXXXXX")
cleanup() {
...
PKG="$STAGE/$PKG_NAME"
/bin/mkdir -p "$PKG/evidence" "$PKG/plugin/hermes"
...
copy_file() {
  ...
  case "$source_path" in
    ...
    *)
      echo "PACKAGE_V172_REFUSED reason=copy-source-outside-plan path=$source_path" >&2
      exit 4
      ;;
  esac
  expected=$(/usr/bin/awk -v target="$relative_path" '$2==target{print $1}' "$SOURCE_PLAN")
  [ -n "$expected" ] || {
    echo "PACKAGE_V172_REFUSED reason=copy-source-unplanned path=$relative_path" >&2
    exit 4
  }
  before=$(sha256 "$source_path")
  [ "$before" = "$expected" ] || {
    echo "PACKAGE_V172_REFUSED reason=copy-source-prehash-drift path=$relative_path" >&2
    exit 4
  }
  [ ! -e "$destination_path" ] && [ ! -L "$destination_path" ] || {
    echo "PACKAGE_V172_REFUSED reason=copy-destination-exists path=$destination_path" >&2
    exit 4
  }
  ...
  [ "$after" = "$expected" ] && [ "$copied" = "$expected" ] || {
    echo "PACKAGE_V172_REFUSED reason=copy-source-or-destination-drift path=$relative_path" >&2
    exit 4
  }
}
...
[ "$(sha256 "$V170_ARCHIVE")" = "$V170_ARCHIVE_SHA256" ] || {
  echo "PACKAGE_V172_REFUSED reason=archival-archive-identity" >&2
  exit 4
}
...
/bin/chmod +x \
  ...
  "$PKG/claim_bundle_verify.py" "$PKG/plugin/hermes/jackal_hermes"
...
JACKAL v1.7.2 — explicit boundary
...
JACKAL v1.7.2 — closed-premise proof identities for Apple Silicon macOS.
...
def refuse(reason: str) -> None:
    raise SystemExit(f"PACKAGE_V172_REFUSED reason=staged-semantic-{reason}")
...
PLUGIN_SELFTEST=$("$PKG/plugin/hermes/jackal_hermes" selftest 2>&1) || {
  echo "PACKAGE_V172_REFUSED reason=staged-plugin-selftest detail=$PLUGIN_SELFTEST" >&2
  exit 4
}
case "$PLUGIN_SELFTEST" in
  *"plugin_hermes.identity_match=true"*) ;;
  *)
    echo "PACKAGE_V172_REFUSED reason=staged-plugin-identity detail=$PLUGIN_SELFTEST" >&2
    exit 4
    ;;
esac
...
RANGE_SMOKE=$("$PKG/jackal-cert-release" x 0 1 "$RANGE_SMOKE_RECEIPT" 2>&1) || {
  echo "PACKAGE_V172_REFUSED reason=staged-range-smoke detail=$RANGE_SMOKE" >&2
  exit 4
}
case "$RANGE_SMOKE" in *"status=formal-bounded"*) ;; *)
  echo "PACKAGE_V172_REFUSED reason=staged-range-status detail=$RANGE_SMOKE" >&2
  exit 4
esac
INT_SMOKE=$("$PKG/jackal-int-cert-release" 0 0 1 2 "$INT_SMOKE_RECEIPT" 2>&1) || {
  echo "PACKAGE_V172_REFUSED reason=staged-int-smoke detail=$INT_SMOKE" >&2
  exit 4
}
case "$INT_SMOKE" in *"status=formal-bounded"*) ;; *)
  echo "PACKAGE_V172_REFUSED reason=staged-int-status detail=$INT_SMOKE" >&2
  exit 4
esac
...
RANGE_VERIFY=$("$PKG/jackal-receipt-verify" \
  ...
  echo "PACKAGE_V172_REFUSED reason=staged-range-replay detail=$RANGE_VERIFY" >&2
  exit 4
}
INT_VERIFY=$("$PKG/jackal-receipt-verify" \
  ...
  echo "PACKAGE_V172_REFUSED reason=staged-int-replay detail=$INT_VERIFY" >&2
  exit 4
}
for replay in "$RANGE_VERIFY" "$INT_VERIFY"; do
  ...
  /usr/bin/printf '%s\n' "$replay" | /usr/bin/grep -F "checker_verdict=ACCEPT" >/dev/null || {
    echo "PACKAGE_V172_REFUSED reason=staged-replay-markers detail=$replay" >&2
    exit 4
  }
done
...
[ ! -e "$FINAL_PKG" ] && [ ! -L "$FINAL_PKG" ] \
  && [ ! -e "$FINAL_TARBALL" ] && [ ! -L "$FINAL_TARBALL" ] || {
  echo "PACKAGE_V172_REFUSED reason=output-appeared-during-build" >&2
  exit 5
}
/bin/mv "$PKG" "$FINAL_PKG"
/bin/mv "$STAGED_TARBALL" "$FINAL_TARBALL"

echo "PACKAGE_V172_BUILD_PASS version=$VER platform=$PLATFORM"
echo "package=$FINAL_PKG"
