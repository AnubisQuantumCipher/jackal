#!/usr/bin/env python3
"""Derive the additive JACKAL v1.7.2 release-manifest plan from live bytes.
...
HEADER = (
    "# JACKAL v1.7.2 pinned release identities (closed-premise range/int "
    "proof identity schema v2 + v1.7.0 computation/claim/gaussian lanes)"
)
...
def write_atomic(path: Path, data: str) -> None:
    descriptor, temporary = tempfile.mkstemp(prefix=".MANIFEST.v172.", dir=path.parent)
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if current != proposed:
            ...
            print("REPIN_V172_CHECK_FAIL")
            return 1
        print(f"REPIN_V172_CHECK_PASS rows={len(build_rows())}")
        return 0
    if args.write:
        write_atomic(MANIFEST, proposed)
        print(f"REPIN_V172_WRITTEN rows={len(build_rows())} manifest={MANIFEST}")
        return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"REPIN_V172_REFUSED detail={str(error)[:512]}", file=sys.stderr)
