#!/usr/bin/env python3
"""JACKAL v1.5.0 sin_rat end-to-end release test.

Exercises the fragment-extension release lane:
  * `sin(x)` on `[1/2, 1/2]` — enclosure contains sin(0.5), width <= 1e-3
  * `sin(x)` on `[-1, 1]`    — full admitted midpoint span
  ...
  * request-relabel (`cos(x)` against a sin cert) — checker rejects
...
pins the `sin_rat_producer` label; identity pins are read from the manifest at
...
WRAPPER = ROOT / "jackal-sin-rat-release"
...
# 1. [1/2, 1/2] — enclosure contains sin(0.5) within 1e-3
enc, detail = _accept(EXPR, "1/2", "1/2")
check("T1-sin-half-point",
      enc is not None
      and float(enc[0]) <= math.sin(0.5) <= float(enc[1])
      and float(enc[1] - enc[0]) <= 1e-3,
      detail)

# 2. [-1, 1] — full admitted midpoint span
enc, detail = _accept(EXPR, "-1", "1")
check("T2-sin-minus1-1",
      enc is not None
      and float(enc[0]) <= math.sin(-1.0)
      and float(enc[1]) >= math.sin(1.0),
      detail)

# 3. Midpoint 3/2 — producer refuses fail-closed (|m| <= 1 lock)
code, _, perr = _emit(EXPR, "1", "2")
check("T3-midpoint-out-of-range-refused",
...
# 5. Cert-bytes tamper: flip one output digit — checker rejects
code, cert, _ = _emit(EXPR, "1/2", "1/2")
tampered = _flip_output_digit(cert)
if code != 0 or tampered == cert:
    check("T5-tamper-cert-refused", False, "no tamper applied")
else:
    code, out_err = _check_cert(tampered, EXPR, "1/2", "1/2")
    check("T5-tamper-cert-refused",
          code != 0 and "REJECT" in out_err,
          out_err.strip().split("\n")[0][:80])

# 6. Request-relabel: valid sin cert, checker asked for cos(x)
code, cert, _ = _emit(EXPR, "1/2", "1/2")
code, out_err = _check_cert(cert, "cos(x)", "1/2", "1/2")
check("T6-relabel-expression-refused",
      code != 0 and "REJECT" in out_err,
      out_err.strip().split("\n")[0][:80])
...
if failures:
    print("VERDICT: FAIL")
    sys.exit(1)
print("VERDICT: PASS — sin_rat fragment extension end-to-end")
